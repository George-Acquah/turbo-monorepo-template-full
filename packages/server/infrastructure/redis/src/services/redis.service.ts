...
import { LOGGER_TOKEN, LoggerPort, RedisPort } from '@workspace/ports';
import { REDIS_CLIENT } from '@workspace/constants';
...
export class RedisService implements RedisPort {
  ...
  /** Atomically increments the integer stored at `key`. Used by CacheService for version bumps. */
  async incr(key: string): Promise<number> {
    return this.redisClient.incr(key);
  }

  /**
   * Sets `key` to `value` only if it doesn't already exist, with a TTL.
   * Returns true if the value was set (lock/value acquired), false if
   * something else already holds it. Used by CacheService for single-flight
   * stampede protection and stale-while-revalidate refresh locking.
   */
  ...
  /** Sets/refreshes the expiry on an existing key. Used to keep lock TTLs accurate if ever renewed. */
