...
import type { PartitionContext, PartitionedQueueConfig } from '@workspace/types';
import { PARTITIONED_QUEUE_CONFIG_TOKEN, PartitionStrategyPort } from '@workspace/ports';
...
export class HashPartitionStrategy implements PartitionStrategyPort {
  ...
  getPartitionKey(baseQueueName: string, ctx: PartitionContext): string {
    ...
    const key = (ctx.key ?? ctx.userId ?? 'global') || 'global';
