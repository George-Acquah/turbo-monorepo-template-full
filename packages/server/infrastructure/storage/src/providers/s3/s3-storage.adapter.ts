...
export class S3StorageAdapter implements StoragePort {
  ...
  getPublicUrl(input: StorageObjectRef): string | null {
    ...
    // When a dedicated public bucket is configured, only objects resolved to
    // that bucket are ever treated as publicly reachable — everything else
    // (private bucket, receipts, lesson video, etc.) falls through to the
    // signed-URL path in getDownloadUrl/getUploadUrl. Without this guard, a
    // publicBaseUrl configured for the public bucket's custom domain would
    // also produce a (non-working, and misleadingly "public") URL for every
    // other bucket, since this method has no other way to know which bucket
    // the custom domain actually serves.
    if (this.config.s3.publicBucket && bucket !== this.config.s3.publicBucket) {
      return null;
    }
  ...
  async readObject(input: StorageObjectRef): Promise<ReadObjectResult> {
    ...
    return { body, contentType: response.ContentType ?? undefined };
  }

  private resolveBucket(bucket?: string): string {
    const value =
      bucket?.trim() || this.config.s3.bucket?.trim() || this.config.defaultBucket.trim();
    if (!value) {
