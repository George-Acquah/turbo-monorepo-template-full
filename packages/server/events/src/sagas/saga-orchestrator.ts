...
import { IdPrefixes, SagaStatus } from '@workspace/constants';
import {
...
export class SagaOrchestrator implements SagaOchestratorPort {
  ...
  async compensateByCorrelationId(correlationId: string): Promise<void> {
    ...
    await this.saga.updateCompensatingByCorrelationId(correlationId);
    ...
    await this.saga.updateCompensatedByCorrelationId(correlationId);
  }
  ...
  private async executeInternal(
    ...
    try {
      const result = await this.transaction.execute<SagaStatePersistence>(async (tx) => {
        // Initialize saga state
        const state: CreateSagaStateInput = {
          id: sagaId,
        ...
        try {
          ...
          for (const step of definition.steps) {
            ...
            if (state.timeoutAt && state.timeoutAt.getTime() < Date.now()) {
              ...
              const persisted = await this.saga.saveTx(tx, state);

              // Compensate what we've done so far
              await this.compensate(sagaId, executedSteps, persisted);
      ...
      const fallbackState: SagaStatePersistence = {
        id: sagaId,
        sagaType: definition.name,
        correlationId,
        ...
        failedStep: executedSteps.at(-1)?.name ?? null,
        data,
        lastError: message,
        startedAt: new Date(),
        completedAt: null,
