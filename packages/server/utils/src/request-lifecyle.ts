...
import { ContextPort } from '@workspace/ports';
...
export function buildHttpRequestMeta(
  ...
  const method = safeContext(ctx, () => ctx.getMethod(), undefined) ?? request?.method;
  ...
  // Deliberately not included: the caller's IP. It stays available
  // internally (RequestContext, logs, ctx.getIp()) for audit purposes, but
  // never gets serialized into a client-facing response envelope — ZAP's
  // "Private IP Disclosure" finding flagged this being echoed back.
...
export function buildWebSocketMeta(
  ...
  // Deliberately not included: the caller's IP — see buildHttpRequestMeta's
