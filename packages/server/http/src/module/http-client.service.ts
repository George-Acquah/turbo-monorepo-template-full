...
import { HttpClientConfig } from '@workspace/types';
import { CONTEXT_TOKEN, ContextPort, HttpPort, LOGGER_TOKEN, LoggerPort } from '@workspace/ports';
import { differenceInMilliseconds, nowMs } from '@workspace/utils/date';
...
export class HttpClientService implements HttpPort {
  ...
  private async request<T>(
    ...
    try {
      ...
      const duration = differenceInMilliseconds(nowMs(), startTime);
  ...
  private isCircuitOpen(key: string): boolean {
    ...
    if (circuit.state === 'open') {
      ...
      if (differenceInMilliseconds(nowMs(), circuit.lastFailure) > CIRCUIT_RESET_TIME) {
