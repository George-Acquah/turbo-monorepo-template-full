...
import { RATE_LIMIT_TOKEN, RateLimitPort } from '@workspace/ports';
import { REDIS_PORT_TOKEN, RedisPort, LOGGER_TOKEN, type LoggerPort } from '@workspace/ports';
import { RedisKeyPrefixes } from '@workspace/constants';
import type { RateLimitRequestContext, RateLimitDecision } from '@workspace/types/contracts';
...
export class RateLimitService extends RateLimitPort {
  ...
  private normalizePrefix(policyPrefix: string): string {
    ...
    if (v === 'email') return `${RedisKeyPrefixes.RATE_LIMIT.COMMS}`;
