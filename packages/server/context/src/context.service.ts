...
import { ContextPort, TransactionEngine } from '@workspace/ports';
...
export class AsyncContextService implements ContextPort {
  ...
  getRequest(): AppRequest | undefined {
    return this.getStore()?.getRequest?.() as AppRequest | undefined;
  }
  ...
  getRawRefreshToken(): string | undefined {
    return this.get('rawRefreshToken');
  }
  ...
  setDeviceId(deviceId: string): boolean {
  ...
  setActor(actor: WorkspaceEventActor): boolean {
    return this.set('actor', actor);
  }

  getTrace(): WorkspaceEventTrace {
    ...
    return {
      ...
      sessionId: this.getSessionIdOptional(),
    };
  }

  setTrace(trace: WorkspaceEventTrace): boolean {
  ...
  isSuperAdmin(): boolean {
    return this.getUser()?.role === 'platform_admin';
  }
