...
import type { RevokePermissionFromRoleUseCaseInput } from '../dto/role.dto';
...
export class RevokePermissionFromRoleUseCase {
  ...
  async execute(input: RevokePermissionFromRoleUseCaseInput): Promise<void> {
    ...
    await this.transactionPort.execute(async (tx) => {
      await this.roleRepo.revokePermission(input.roleId, input.permissionId, tx as DatabaseTx);

      await this.publisher.publishWithTransaction(tx, {
        eventType: IdentityEvents.PERMISSION_REVOKED_FROM_ROLE,
